...
fn main() -> io::Result<()> {
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let (area_gauges, _) = area_compare.vsplits([1; 20]);
            for (idx, area_gauge_idx) in area_gauges.into_iter().enumerate() {
                f.render_widget(
                    LineGauge::default()
                    .ratio((0.05 * idx as f64).min(1.0))
                    .gauge_style(Style::default().fg(Color::Rgb((255 * idx / 30) as u8, 127, 127)).bg(Color::Black).add_modifier(Modifier::BOLD))
                    .line_set(symbols::line::NORMAL)
                    .label(Spans::from(Span::raw(format!("job #{:02}", idx))))
                    ,
                    area_gauge_idx,
                );
            }
            ...
            let slider = Slider::new(offset_y, contents.len()).slider_color(Color::Red);
            f.render_widget(
                // Gauge::default()
            ...
            let slider = Slider::new(offset_x, text.width()).direction(Direction::Horizontal).slider_color(Color::Green);
            f.render_widget(
                Paragraph::new(Text::from(format!("{} {}: {:?}", offset_x, offset_y, slider.positon(area_hslider))))
